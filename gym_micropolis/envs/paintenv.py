...
class MicropolisPaintEnv(core.Env):
    ...
    def setMapSize(self, size, max_step=None, rank=None, print_map=False, PADDING=0, static_builds=True, 
        ...
        self.micro = MicropolisPaintControl(self.MAP_X, self.MAP_Y, PADDING, parallel_gui=parallel_gui, rank=rank)
        ...
        ac_low = np.zeros((self.num_tools, self.MAP_X, self.MAP_Y))
        ac_high = np.ones((self.num_tools, self.MAP_X, self.MAP_Y))
  ...
  #             self.
    ...
    def step(self, a, static_build=True):
        ...
        if self.micro.player_builds:
            print("Player interaction not implemented for Micropolis Paint Environment")
